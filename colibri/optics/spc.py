...
class SPC(BaseOpticsLayer):
    ...
    """
    def __init__(self, input_shape, n_measurements=256, trainable=True, initial_ca=None, binary=False, **kwargs):
        r"""       
        ...
        if initial_ca is None:
            initializer = torch.randn((n_measurements, M*N), requires_grad=trainable)
        else:
            initializer = initial_ca
        
        #Add parameter CA in pytorch manner
        ca = torch.nn.Parameter(initializer, requires_grad=trainable)
