...
class PnP_ADMM(nn.Module):
    ...
    def forward(self, y, x0=None, verbose=False, freq=None, xgt=None):
        ...
        for i in range(self.max_iters):
            ...
            u_t = u_t + x_t - v_t
            if xgt is not None:
                psnr_ = colibri.metrics.psnr(v_t, xgt, 1)
                print(f"PSNR: {psnr_}")
...
class MultiRegPnP_ADMM(nn.Module):
    r"""
    Plug-and-Play (PnP) algorithm with Alternating Direction Method of Multipliers (ADMM) formulation.
    ...
    def __init__(self, acquisition_model, fidelity=L2(), prior=Sparsity("dct"), solver="close", max_iters=20, _lambda=0.1, rho=0.1, alpha=0.01):
        ...
        super(MultiRegPnP_ADMM, self).__init__()
    ...
    def forward(self, y, x0=None, verbose=False, freq=None, xgt=None):
        ...
        if self.solver == "close":
            ...
            GradientDescent = lambda x, xt: x - self.get_gradient(x,y)  - self.rho * (x - xt)
            x_solver = GradientDescent
    ...
    def get_gradient(self, x, y):
        r"""Computes the gradient of the fidelity term.

        Args:
            x (torch.Tensor): The current estimate of the solution.
            y (torch.Tensor): The measurement data.

        Returns:
            torch.Tensor: The gradient of the fidelity term.
        """
        grad = 0
        for j in range(len(self.alpha)):
                    
                    # If the fidelity term is a learned model, we need to pass the forward model
            grad += self.fidelity[j].grad(x, y[j], self.H[j])*self.alpha[j]
